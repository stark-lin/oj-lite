...
更新版如下，已把 **Challenge** 调整为包含 `+ - * / %` 的综合变量应用题：

## Week 01：变量

| 项目   | 内容                                  |
| ---- | ----------------------------------- |
| 核心主题 | 变量是保存数据、记录中间结果的盒子                   |
| 数学基础 | 四则运算、面积、总价、平均值、差值、找零、取余             |
| 编程内容 | `local`、赋值、变量更新、临时变量、运算符优先级         |
| 算法内容 | 顺序计算、变量交换、状态更新、多步结算                 |
| 场景   | 游戏血量、操场面积、文具购物、金币变化、跑步累计、换座位、礼包折扣结算 |
| 目标   | 学生能用变量拆解真实应用题，并用加、减、乘、除、取余表达完整计算过程  |
...
| 07 | Challenge | 两名同学换座位后更新座位统计卡     |
| 08 | Challenge | 游戏角色连续战斗后的最终血量结算    |
| 09 | Challenge | 商店礼包活动后的折扣、打包费和找零结算 |
